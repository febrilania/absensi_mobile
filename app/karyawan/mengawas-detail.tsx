...
export default function MengawasDetail() {
  ...
  const handleSelesaiUjian = async () => {
    try {
        ...
        Alert.alert("Berhasil", "Ujian telah diselesaikan!", [
          {
            ...
            onPress: () => {
              if (source === "code") {
                // ✅ Kalau datang dari Code, balik ke Code
                router.replace({
                  pathname: "/(tabsKaryawan)/code",
                });
              } else {
                // ✅ Kalau dari Scan, balik ke Scan
                router.replace({
      ...
      Alert.alert("Error", "Terjadi kesalahan saat menyelesaikan ujian.");
    } finally {
      setFinishing(false);
    }
  };
